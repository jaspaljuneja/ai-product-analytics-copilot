...
The MVP uses a **SQLite relational database** containing synthetic product analytics data. The dataset is designed to simulate a consumer digital product with users, products, behavioral events, and completed orders.

### Database Overview

| Table      | Purpose                         | Approx. Records |
| ---------- | ------------------------------- | --------------: |
| `users`    | User/customer attributes        |          10,000 |
| `products` | Product catalog                 |              50 |
| `events`   | User behavioral events          |         220,000 |
| `orders`   | Completed purchases and revenue |          11,523 |

The dataset covers **January through September 2026**, with September representing an incomplete period.
...
| `device`      | TEXT    | Primary device type      |
| `age_group`   | TEXT    | User age group           |
...
The `events` table is the primary source for behavioral and funnel analysis.
...
| `revenue`    | REAL    | Revenue generated by the order |

**Primary key:** `order_id`

The `orders` table is the primary source for revenue and transaction-level analysis.

---

## Relationships

The database uses the following logical relationships:
...
These relationships allow the Copilot to answer questions such as:

* Which device has the highest conversion rate?
* Which product category generates the most revenue?
* How does revenue vary by month?
* Which products have the highest sales?
* Did a particular segment contribute to a conversion decline?
* What happened to the funnel during a specific period?

## Business Metric Definitions

The Copilot uses explicit business definitions rather than allowing the LLM to invent metric logic.

### Conversion Rate

```text
Unique users with a purchase event
---------------------------------- × 100
Unique users with any event
```

A user is counted once, regardless of the number of events they generate.
...
A unique customer identified by `user_id`.

### Product

A product identified by `product_id`.

### Category

The product category stored in the `products` table.

### Device

The user's device type stored in the `users` table.

Current synthetic device values include:
...
The schema intentionally separates **behavioral data** from **transactional data**.
...
> What did users buy and how much revenue was generated?
...
> What are they interacting with or purchasing?

This structure gives the AI Copilot enough relational context to perform meaningful product analytics while remaining small enough for an MVP.

## Data Generation

The database is generated using `src/generate_data.py`.

The synthetic dataset includes a deliberately introduced **August conversion-rate anomaly** so the Copilot can be tested against a known business scenario rather than only answering descriptive questions.

This allows the project to demonstrate a complete analytics workflow:

```text
Synthetic Data
      ↓
Known Business Anomaly
      ↓
